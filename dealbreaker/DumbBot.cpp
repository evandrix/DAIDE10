...
{
	send_name_and_version_to_server("Dealbreaker", "10");
}
...
void DumbBot::process_mdf_message( TokenMessage &incoming_message )
{
	...
	// construct single instance of negotiator class thread wrapper
	negotiator = Negotiator::getInstance(this);
}
...
void DumbBot::process_sco_message( TokenMessage &incoming_message )
{
	...
	for ( province_counter = 0; province_counter < m_map_and_units->number_of_provinces; province_counter++ )
	{
		...
		{
			m_power_size[ MapAndUnits::get_power_index( m_map_and_units->game_map[ province_counter ].owner ) ]++;
		}
	}
		...
		if ( adjacent_unit_iterator != m_map_and_units->units.end() )
		{
			...
			if ( ( m_power_size[ adjacent_unit_iterator->second.nationality ] > defence_value )
				&& ( adjacent_unit_iterator->second.nationality != m_map_and_units->power_played.get_subtoken()
					|| negotiator->is_allied_with(adjacent_unit_iterator->second.nationality)
				)
			)
			{
...
void DumbBot::process_frm_message( TokenMessage &incoming_message )
{
	_printf("DumbBot::process_frm_message - %s\n", incoming_message.get_message_as_text().c_str());

	// parse incoming message
	Token from_power;
	TokenMessage press_message, message_id;
	...
	// Level 10 - only understand PRP-TokenMessages
	if ( ( press_message.get_token( 0 ) == TOKEN_PRESS_PRP ) )
	{
		// Should be process PRP token
	    TokenMessage outgoing_message = TOKEN_COMMAND_YES & press_message;
		send_press_to_server(from_power, outgoing_message);
		_printf("DumbBot::process_frm_message - %s\n", outgoing_message.get_message_as_text().c_str());
	}
	else if ( ( press_message.get_token( 0 ) == TOKEN_COMMAND_YES ) )
	{
		process_yes_frm_message(from_power,press_message);
	}
	else if ( ( press_message.get_token( 0 ) == TOKEN_COMMAND_REJ ) )
	{
		process_rej_frm_message(from_power,press_message);
	}
	else if ( ( press_message.get_token( 0 ) == TOKEN_PRESS_BWX ) )
	{
		// They said none of our business so count as rejection
		process_rej_frm_message(from_power,press_message);
	}
}

void DumbBot::process_yes_frm_message(Token &from_power, TokenMessage &incoming_message) 
{
	_printf("DumbBot::process_yes_frm_message - %s\n", incoming_message.get_message_as_text().c_str());

	// parse incoming message
	TokenMessage press_message = incoming_message.get_submessage( 1 );
	...
	else if ( ( press_message.get_token( 0 ) == TOKEN_PRESS_NAR ) )
	{
		// No arrangement - next token is PRP
	}
	else if ( ( press_message.get_token( 0 ) == TOKEN_COMMAND_NOT ) )
	{
		// Logical opposite of the arrangement - next token is PRP
	}
}

void DumbBot::process_rej_frm_message(Token &from_power, TokenMessage &incoming_message) 
{
	_printf("DumbBot::process_rej_frm_message - %s\n", incoming_message.get_message_as_text().c_str());
...
void DumbBot::process_prp_message( TokenMessage &incoming_message )
{
	OutputDebugString("DumbBot::process_prp_message\n");

	//Level 10 possible tokens:
	//PCE
	//ALY
	//DRW
	//SLO
	...
	for ( province_counter = 0; province_counter < m_map_and_units->number_of_provinces; province_counter++ )
	{
		if ( m_map_and_units->game_map[ province_counter ].is_supply_centre )
		{
			...
			else
			{
				...
				if (negotiator->is_allied_with(m_map_and_units->game_map[ province_counter ].owner))
				{
					m_attack_value [ province_counter ] = 0;
				}
				else
				{
					m_attack_value[ province_counter ] = m_power_size[ MapAndUnits::get_power_index( m_map_and_units->game_map[ province_counter ].owner ) ];
				}
			}
		}
	}
...
void DumbBot::generate_debug( void )
{
	...
	if ( fp == NULL )
	{
		AfxMessageBox("Couldn't open debug file " + filename );
	}
	else
	{
...
void DumbBot::check_for_wasted_holds( MOVING_UNIT_MAP &moving_unit_map )
{
		...
		our_unit_iterator++ )
	{
		...
		if ( unit->order_type == MapAndUnits::HOLD_ORDER )
		{
			...
			{
				// Support it
				log_debug("Overriding hold order in %s with support from %s to %s",
					TokenTextMap::instance()->m_token_to_text_map[ m_map_and_units->game_map[ unit->coast_id.province_index ].province_token ].c_str(),
...
void DumbBot::log_debug( char *error, ... )
{
//#ifdef _DEBUG
//	va_list arg_list;
