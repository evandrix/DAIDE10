...
public:
	...
	{
		int power_index = MapAndUnits::MAX_POWERS;
	...
	typedef struct
	{
		...
		PROVINCE_INDEX other_source_province;	// Province support or convoy is from (if moving)
												...
												// (for support to hold, province supported unit is in)
		PROVINCE_INDEX other_dest_province;		// Province support or convoy is to
	...
	typedef struct PROVINCE_DETAILS
	{
		// to allow for set<PROVINCE_DETAILS>
		bool operator<(const PROVINCE_DETAILS& other) const
		{
			return province_token < other.province_token;
		}
		
		LPCTSTR toString() {
			TokenTextMap* map = TokenTextMap::instance();
			return map->m_token_to_text_map[ province_token ].c_str();
		}

		Token province_token, owner;
		bool province_in_use, is_supply_centre, is_land;
	...
	COAST_ID get_build_or_disband( PROVINCE_INDEX province );
