...
int MapAndUnits::set_units( TokenMessage &now_message )
{
	...
	if ( number_of_provinces != NO_MAP )
	{
		...
		else
		{
			...
			// If it's a winter turn, then update sc ownership, to set up builds/disbands count
...
MapAndUnits::COAST_ID MapAndUnits::get_build_or_disband( PROVINCE_INDEX province )
{
	COAST_ID min_coast;					// Minimum coast for this province
	BUILDS_OR_DISBANDS::iterator matching_unit_iterator;	
										// Iterator to unit already in the build list which is in the same location
	COAST_ID unit;						// Unit already in the build list which is in the same location

	min_coast.province_index = province;
	min_coast.coast_token = Token( 0, 0 );

	matching_unit_iterator = our_winter_orders.builds_or_disbands.lower_bound( min_coast );

	if ( ( matching_unit_iterator != our_winter_orders.builds_or_disbands.end() )
	  && ( matching_unit_iterator->first.province_index == province ) )
	{
		unit = matching_unit_iterator->first;
	}
	else
	{
		unit.province_index = NO_PROVINCE_INDEX;
	}
